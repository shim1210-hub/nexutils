...
export default function DateTimeToolsTool() {
  ...
  const [secondInput, setSecondInput] = useState(new Date(Date.now() + 7 * 86400000).toISOString().slice(0, 10));
  ...
  const output = useMemo(() => {
    ...
    if (mode === "timezone") {
      ...
      return timezoneDirection === "utc-to-kst"
        ? `KST: ${date.toLocaleString("ko-KR", { timeZone: "Asia/Seoul" })}`
        : `UTC: ${new Date(date.getTime() - 9 * 3600000).toISOString()}`;
    }
  ...
  return (
    <div className="tool-form form-workspace date-workspace">
      ...
      <div className="control-bar">
        ...
        {mode === "timezone" ? <select onChange={(event) => setTimezoneDirection(event.target.value)} value={timezoneDirection}><option value="utc-to-kst">UTC → KST</option><option value="kst-to-utc">KST → UTC</option></select> : null}
        {mode === "uuid" ? <input onChange={(event) => setUuidCount(event.target.value)} placeholder="생성 개수" type="number" value={uuidCount} /> : null}
      </div>
